...
This KPI catalog defines the procurement and SAP Activate project KPIs for the SAP Activate ERP Procurement Analytics project. The data model is implemented for the current Phase 4 scope, but not every KPI in this catalog has been implemented as a separate SQL query file yet.

The current repository includes SQLite views and generator validation logic for item fulfillment, delivery performance, and invoice three-way matching. Spend, payment, data readiness, and dashboard-ready SQL query packages remain planned work.
...
- Future SQL query files should use the current schema and views as source objects rather than duplicating fulfillment or matching logic.
